...
public class Arene extends UnicastRemoteObject implements IAreneIHM, Runnable {
	...
	public boolean lanceBouleDeFeu (int refRMI,int refRMIAdv) throws RemoteException{
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			if (distance <= Constantes.DISTANCE_MIN_INTERACTION_GRANDE) {
				...
				if (pers.estVivant() && persAdv.estVivant()) {
					...
					new BouleDeFeu(this, client, clientAdv).interagir();
					personnages.get(refRMI).executeAction();
	...
	@Override
	public boolean LanceVol(int refRMI, int refRMIAdv) throws RemoteException {
		boolean res = false;
		...
		if (personnages.get(refRMI).isActionExecutee()) {
			...
			if (distance <= Constantes.DISTANCE_MIN_INTERACTION) {
				...
				if (pers.estVivant() && persAdv.estVivant()) {
					...
					new Vol(this, client, clientAdv).interagir();
					personnages.get(refRMI).executeAction();
